...
final class ServiceFundingTests: XCTestCase {
    ...
    func testAllFiveFundableServicesTrackIndependently() {
        var funding = ServiceFunding()
    ...
    /// Zoned land, roads, and `.empty` aren't fundable — `setLevel` is a
    /// harmless no-op for them rather than trapping, so a caller iterating
    /// every `ZoneType` doesn't need to filter down to the fundable ones
    /// first.
