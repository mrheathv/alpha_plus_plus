...
///
/// It is handed a closure that draws the frame at a given number of seconds in
/// — the Metal renderer, driven at a fixed step — which is what makes it work
/// with no window and no display, the same reason `Capture Screenshot…` writes
/// its own PNG instead of calling `screencapture`.
///
...
final class SceneRecorder {
    ...
    /// Draws the frame at a given number of seconds in.
    ...
    /// Records `seconds` of frames, writing every one to the movie and holding
    /// back every `keepEvery`th for the filmstrip (by default, eight in all).
    @discardableResult
